...
def sim(z_i, z_j):
    ...
    norm_z_j = torch.linalg.norm(z_j, dim=-1)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k_kN = -torch.log(
            torch.exp(sim(z_k, z_k_N) / tau) /
            (torch.sum(torch.exp(sim(z_k, out) / tau)) - torch.exp(sim(z_k, z_k) / tau))
        )
        l_kN_k = -torch.log(
            torch.exp(sim(z_k_N, z_k) / tau) /
            (torch.sum(torch.exp(sim(z_k_N, out) / tau)) - torch.exp(sim(z_k_N, z_k_N) / tau))
        )
...
def sim_positive_pairs(out_left, out_right):
    ...
    left_norm = torch.linalg.norm(out_left, dim=-1, keepdim=True)
    right_norm = torch.linalg.norm(out_right, dim=-1, keepdim=True)
    pos_matrix = out_left @ out_right.T / (left_norm @ right_norm.T)
    pos_pairs = torch.diag(pos_matrix).view(-1, 1)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = out @ out.T / (norm_out @ norm_out.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.sum(exponential, dim=1).view(-1, 1)  # [2*N, 1]
